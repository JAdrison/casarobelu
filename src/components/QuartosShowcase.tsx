...
import { easeLuxe, stagger, fadeUp, viewportOnce } from "@/lib/motion";

const items = [
  { src: quarto1, label: "Suítes Master", desc: "Ar-condicionado, cama casal e enxoval premium." },
  { src: quarto2, label: "Quartos Familiares", desc: "Conforto para receber a família inteira." },
  { src: quarto3, label: "Espaços Compartilhados", desc: "Áreas de descanso integradas à natureza." },
];

export const QuartosShowcase = () => (
  <section
    className="relative px-5 sm:px-6 -mt-12 md:-mt-24 z-10"
    aria-label="Showcase de ambientes"
  >
    <motion.div
      ...
      {items.map((it) => (
        <motion.figure key={it.label} variants={fadeUp} className="group">
          ...
          >
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-dark-text/55 via-transparent to-transparent" />
            <figcaption className="absolute inset-x-0 bottom-0 p-5 sm:p-6 text-off-white">
              ...
              <p
                className="font-serif-italic italic text-sm sm:text-base mt-1.5 text-off-white/85 leading-snug"
              >
                {it.desc}
              </p>
            </figcaption>
